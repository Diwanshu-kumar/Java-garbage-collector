...
public class GarbageCollector {
	 ...
	 * methods.
	 */
	public Digraph heapMemory;
	public Deque<Vertex> stackMemory;
	public HashMap<String, Vertex> markedObjects;
	...
	{
        return isInHeapMemory(id) || isInStackMemory(id);
	}
	...
	public boolean isInStackMemory(String id)
	{
		...
		if (stackSize > 0)
		{
			for (int i = 0; i < stackSize; i++)
			{
				...
				// In stackMemory.
				if (Objects.equals(id, temp.getID()))
	...
	public boolean isMarked(String id)
	{
        ...
        isMarked = markedObjects.containsKey(id);
	...
	public void mark()
	{
		...
		if (stackUse() > 0 || heapUse() > 0)
		{
			...
			Stack<Vertex> unprocessed = new Stack<>();

			// Queue of stackMemory objects
			Queue<Vertex> objectQueue = new LinkedList<>();
				...
				while (!unprocessed.isEmpty())
				{
					...
					if (!isMarked(obj1.getID()))
					{
                        ...
                        for (String vertex : vertices) {
                            Vertex obj2 = heapMemory.getVertex(vertex);

                            // If there is an edge between the two objects, and they aren't the same object.
                            if (heapMemory.adjacent(obj1.getID(), obj2.getID()) && !Objects.equals(obj1.getID(), vertex))
	...
	public void removeReference(String fromId, String toId)
	{
		...
		if (isInStackMemory(fromId))
		{
			...
			if (isInHeapMemory(toId))
			{
				...
				if (tempStack.adjacent(heapMemory.getVertex(toId)))
		...
		if (isInHeapMemory(fromId))
		{
			...
			if (isInStackMemory(toId))
			{
				...
				// Has a reference between the two objects.
				if (heapStack.adjacent(tempStack))
					heapStack.removeNeighbour(tempStack);
			}
			// Heap -> Heap.
			if (isInHeapMemory(toId))
				if (heapMemory.adjacent(fromId, toId))
		...
		if (heapUse() > 0)
		{
            ...
            for (String vertex : vertices)
                if (!markedObjects.containsKey(vertex))
                    heapMemory.removeVertex(vertex);
		}
	}
